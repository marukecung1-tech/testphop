      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
          {CHARACTERS.map((char, index) => (
            ...
            >
              <div 
                className="w-full aspect-[4/5] rounded-[2.5rem] flex items-center justify-center p-6 transition-all shadow-lg group-hover:shadow-2xl relative overflow-hidden border-8"
                style={{ 
              ...
              >
                ...
                {/* 
                  🖼️ หมายเหตุสำหรับรูปภาพตัวละคร (Character Images):
                  รูปที่โชว์ในหน้านี้ถูกดึงมาจากไฟล์ `src/constants.ts` อัตโนมัติ 
                  ให้คุณไปแก้ชื่อไฟล์ภาพที่ `src/constants.ts` ได้เลย (ไม่ต้องแก้โค้ดแท็ก img ด่านล่างนี้)
                */}
                <motion.img 
                  src={char.image} 
                  alt={char.name}
